...
//		if ( ! isset( $from_email ) ) {
		// Use round-robin to select a connection
		$selectedConnection = fluentMailRoundRobinConnection();

		if ( ! empty( $selectedConnection['sender_email'] ) ) {
			$from_email = $selectedConnection['sender_email'];
		} else {
...
//		if ( ! isset( $from_name ) ) {
		if ( $selectedConnection && ! empty( $selectedConnection['sender_name'] ) ) {
			$from_name = $selectedConnection['sender_name'];
		} else {
 ...
 *
 * @return array The next connection to use
 */
function fluentMailRoundRobinConnection() {

	$isEnabled = get_option('fluent_smtp_round_robin_enabled', 0);
	if (!$isEnabled) {
	...
	// Filter to only return active connections
	$activeConnections = [];
	...
	// Get today's email count and check limits
	$today = date('Y-m-d');
	$dailyCounts = get_option('fluent_smtp_daily_counts', []);
	$todayCounts = isset($dailyCounts[$today]) ? $dailyCounts[$today] : [];

	// Find connections that haven't reached their daily limit
	$availableConnections = [];
	foreach ($activeConnections as $key => $connection) {
		$limit = isset($dailyLimits[$key]) ? intval($dailyLimits[$key]) : 0;
		$count = isset($todayCounts[$key]) ? intval($todayCounts[$key]) : 0;

		// If limit is 0 (unlimited) or count is below limit, connection is available
		if ($limit === 0 || $count < $limit) {
			$availableConnections[$key] = $connection;
		}
	}

	// If no connections are available (all at their limits), use default connection
	if (empty($availableConnections)) {
		if (defined('WP_DEBUG') && WP_DEBUG) {
			error_log("FluentSMTP Round-Robin: All connections have reached their daily limits");
		}
		return fluentMailDefaultConnection();
	}

	// Get round-robin data
	$data = get_option('fluent_smtp_round_robin_data', [
		'index' => 0,
		'count' => 0
	]);

	// Get available connection keys
	$connectionKeys = array_keys($availableConnections);

	// Find the current index within available connections
	$currentIndex = isset($data['index']) ? $data['index'] : 0;
	$availableIndex = 0;

	// If current connection is in available connections, use its index
	$allConnectionKeys = array_keys($activeConnections);
	if (isset($allConnectionKeys[$currentIndex])) {
		$currentConnectionKey = $allConnectionKeys[$currentIndex];
		$availableIndex = array_search($currentConnectionKey, $connectionKeys);

		// If current connection is not available, use the first available
	...
	// If we've sent 2 emails with this connection, move to the next one
	if ($count > 2) {
		$availableIndex = ($availableIndex + 1) % count($connectionKeys);
		$count = 1;
	}

	// Update the global index based on the available connection
	$selectedKey = $connectionKeys[$availableIndex];
	$globalIndex = array_search($selectedKey, $allConnectionKeys);
	...
	// Get the connection
	$connection = $availableConnections[$selectedKey];

	// Update today's count for this connection
	$todayCounts[$selectedKey] = isset($todayCounts[$selectedKey]) ? $todayCounts[$selectedKey] + 1 : 1;
	$dailyCounts[$today] = $todayCounts;
	update_option('fluent_smtp_daily_counts', $dailyCounts);

	// Optional debug log
	if (defined('WP_DEBUG') && WP_DEBUG) {
		error_log("FluentSMTP Round-Robin: Using connection {$selectedKey} (Email {$count} of 2)");
	}

	// Update connection_id in the provider settings for logging
	$providerSettings = $connection['provider_settings'];
	$providerSettings['connection_id'] = $selectedKey;
 ...
 * This should be called once per day, for example via WP Cron
 */
function fluentMailResetDailyCountsIfNeeded() {
	$lastReset = get_option('fluent_smtp_last_daily_reset', '');
	$today = date('Y-m-d');

	// If we haven't reset today
	if ($lastReset !== $today) {
		// Don't completely reset, just ensure today exists as a key
		$dailyCounts = get_option('fluent_smtp_daily_counts', []);
		...
		update_option('fluent_smtp_last_daily_reset', $today);
	}
}

/**
 * Add a daily cron to reset counters
 */
add_action('init', function() {
	// Check if counters need to be reset for today
	fluentMailResetDailyCountsIfNeeded();
});

// Schedule a daily event to check and reset counters
if (!wp_next_scheduled('fluentmail_daily_reset_check')) {
	wp_schedule_event(time(), 'daily', 'fluentmail_daily_reset_check');
}

add_action('fluentmail_daily_reset_check', 'fluentMailResetDailyCountsIfNeeded');

/**
 * Clean up old daily counts (keep only the last 30 days)
 */
function fluentMailCleanupOldDailyCounts() {
	$dailyCounts = get_option('fluent_smtp_daily_counts', []);

	// Keep only the last 30 days
	$cutoff = date('Y-m-d', strtotime('-30 days'));
	foreach (array_keys($dailyCounts) as $date) {
		if ($date < $cutoff) {
			unset($dailyCounts[$date]);
		}
	}
...
if (!wp_next_scheduled('fluentmail_cleanup_daily_counts')) {
	wp_schedule_event(time(), 'weekly', 'fluentmail_cleanup_daily_counts');
}

add_action('fluentmail_cleanup_daily_counts', 'fluentMailCleanupOldDailyCounts');